...
Revision ID: 696d5fb2f6e0
Revises: 
Create Date: 2025-02-19 17:29:50.007122
...
def upgrade() -> None:
    ...
    sa.Column('parsing_result_id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['confirmation_file_id'], ['confirmation_files.file_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('parsing_result_id')
    )
